...
def process_beneficiary(df_ben,src_name):
    ...
    df_tab1.to_excel(writer, startrow=0,sheet_name='Active Employees List', index=False)
    writer.save()
    ...
    # writer = pd.ExcelWriter(file_path, engine='xlsxwriter', date_format='mm/dd/yyyy')
    # df_tab1.to_excel(writer, "Active Employee Report", startrow=0, columns=result_excel_headers_actemp, index=False)
    # writer.save()


    #    if 'Date' in y:
    #         # df_tab1[y] = df_ben[x].strftime("%m%d%y")
    #         print(df_tab1[y])
    # quit()
            # date.today().strftime("%m%d%y")
    
    # df_tab1 = df_tab1.sort_values(by=['Beneficiary Name'],ascending=True)
...
def process_casefile(df_case,src_name):
    ...
    #             df_tab2[y] = ""
    #         else:
    #             df_tab2[y] = pd.to_datetime(df_tab2[y], format='%m%d%y', errors='coerce').dt.date
    # df_tab2.columns = result_excel_headers_niv #changing dataframe all column names
    # writer = pd.ExcelWriter(file_path, engine='xlsxwriter',date_format='mm/dd/yyyy')
    # df_tab2.to_excel(writer, "Active Employee Report", startrow=0, columns=result_excel_headers_niv, index=False)
    # writer.save()
    ...
    #                 df_tab3[y] = ""
    #             else:
    #                 df_tab3[y] = pd.to_datetime(df_tab3[y], format='%m%d%y', errors='coerce').dt.date
    # df_tab3.columns = result_excel_headers_perm #changing dataframe all column names
    # writer = pd.ExcelWriter(file_path, engine='xlsxwriter',date_format='mm/dd/yyyy')
    # df_tab3.to_excel(writer, "Active Employee Report", startrow=0, columns=result_excel_headers_perm, index=False)
    # writer.save()
    ...
    #             df_tab4[y] = ""
    #         else:
    #             df_tab4[y] = pd.to_datetime(df_tab4[y], format='%m%d%y', errors='coerce').dt.date
    # df_tab4.columns = result_excel_headers_pr #changing dataframe all column names
    # writer = pd.ExcelWriter(file_path, engine='xlsxwriter',date_format='mm/dd/yyyy')
    # df_tab4.to_excel(writer, "Active Employee Report", startrow=0, columns=result_excel_headers_pr, index=False)
    # writer.save()
    ...
    #             df_tab5[y] = ""
    #         else:
    #             df_tab5[y] = pd.to_datetime(df_tab5[y], format='%m%d%y',errors='coerce').dt.date
    # df_tab5.columns = result_excel_headers_h1b #changing dataframe all column names
    # writer = pd.ExcelWriter(file_path, engine='xlsxwriter',date_format='mm/dd/yyyy')
    # df_tab5.to_excel(writer, "Active Employee Report", startrow=0, columns=result_excel_headers_h1b, index=False)
    # writer.save()
    # writer.close()
    ...
    writer.book = book
    df_tab2.to_excel(writer,startrow=0,sheet_name='NIV Cases', index=False)
    df_tab3.to_excel(writer,startrow=0,sheet_name='PERM Cases', index=False)
    df_tab4.to_excel(writer,startrow=0,sheet_name='PR Cases', index=False)
    df_tab5.to_excel(writer,startrow=0,sheet_name='H-1B Cap Cases', index=False)
    ...
    for x in range(5):
        ...
        for d in (ws[1]):
            if 'Date' in d.value:
                ...
                df_tab2[d.value].style = NamedStyle(name='custom_datetime', number_format='DD/MM/YYYY')

       
        # if 'Date' in xx:
            
        #     ws['xx'].style = NamedStyle(name='custom_datetime', number_format='DD/MM/YYYY')
            
        for y in range(rows):
            for z in range(cols):
                if y == 0:
                
                    ws.cell(row=y+1, column=z+1).font = Font(size = 12,color = 'FFF000')

                    ws.cell(row=y+1, column=z+1).alignment=Alignment(wrap_text=False, horizontal="justify", vertical="justify")
                    
                    ws.cell(row=y+1, column=z+1).alignment=Alignment(wrap_text=True, horizontal="justify", vertical="justify")
                    ...
                    ws.cell(row=y+1, column=z+1).font= Font(size = 11)
        ...
        for cl in range(cols+1):
            ...
            if cl <= cols:
                ...
                # ws.row_dimensions[(rows+1)].height = 8
...
def start():
    for name in glob.glob('Source Data/Active Beneficiary*'):
        ...
        # quit()
        process_beneficiary(df_ben, src_name)
    ...
    for name in glob.glob('Source Data/Open process Data*'):
            ...
            process_casefile(df_case,src_name)
