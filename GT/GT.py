...
def visaBulletinCheck():
    ...
    if results is None:
        ...
        Update_visaBulletin(results[1]) #returns 1st column of the fetched row column
...
def update_log(client_name, status, file1, file2, e ):
    try:
        ...
        sheet = book2.active                
        sheet.append((today, week_day, client_name, 'Processed', status, file1, file2, e) )
...
def send_email(filename, results, filename2, send_exp_report, client_short_name):
    ...
    if length > 0:
        try:
            ...
            update_log(str(client_short_name), 'Not Sent', filename, filename2, e )
